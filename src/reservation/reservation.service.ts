...
export class ReservationService {
  ...
  async checkSeats(
    reservations: CreateReservationDto[] | ReservationEntity[],
  ): Promise<boolean> {
    const enoughSeats = await Promise.all(
      reservations.map((r) => {
        return this.screeningService.isThereAvailableSeats(
          r.screeningId,
          r.products
            .map((r) => r.number)
    ...
    return !enoughSeats.some((enough) => !enough);
  }
}
